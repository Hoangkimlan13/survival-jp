...
function primeSilentOutput(ctx: AudioContext) {
  if (didPrime) return
...
function withReadyAudio(play: (ctx: AudioContext) => void) {
...
export function playCorrect() {
  withReadyAudio(ctx => {
    ...
    g2.gain.exponentialRampToValueAtTime(0.09, now + 0.03)
...
export function playWrong() {
  withReadyAudio(ctx => {
    ...
    hitGain.gain.exponentialRampToValueAtTime(0.22, now + 0.01)
...
export function playStorySound(trigger?: string) {
  withReadyAudio(ctx => {
    ...
    if (kind === "good") {
      playSequence(ctx, [440, 523, 659], "triangle", now, 0.28, 0.16, 1400, 1.2)
      playSequence(ctx, [880, 988], "sine", now + 0.1, 0.28, 0.08, 1800, 0.9)
      playSequence(ctx, [220], "sine", now, 0.9, 0.035, 400, 0.7)
    } else if (kind === "bad") {
      playSequence(ctx, [220, 196, 174], "triangle", now, 0.34, 0.14, 900, 1.8)
      playSequence(ctx, [130], "sine", now + 0.14, 0.34, 0.09, 600, 1.3)
      playSequence(ctx, [280, 262], "square", now + 0.04, 0.24, 0.08, 800, 1.1)
    } else {
      playSequence(ctx, [330, 392, 440], "triangle", now, 0.3, 0.145, 1200, 1.1)
      playSequence(ctx, [660], "sine", now + 0.1, 0.3, 0.095, 1500, 1)
      playSequence(ctx, [220], "sine", now + 0.12, 0.3, 0.05, 500, 1)
    }
  })
}
...
export function playFinalSound() {
  withReadyAudio(ctx => {
    ...
    playSequence(ctx, [523, 659, 784, 880], "triangle", now, 0.32, 0.2, 1400, 1.1)
    playSequence(ctx, [880, 988, 1047, 1175], "sine", now + 0.12, 0.36, 0.14, 1800, 0.9)
    playSequence(ctx, [440, 523], "square", now + 0.08, 0.75, 0.03, 600, 0.9)
    playSequence(ctx, [660, 784, 880], "triangle", now + 0.45, 0.32, 0.18, 1500, 1)
    playSequence(ctx, [988, 1175, 1318], "sine", now + 0.9, 0.42, 0.08, 1600, 1)
    playSequence(ctx, [523], "triangle", now + 1.2, 0.6, 0.05, 1200, 1.1)
  })
}
...
export function playEndingSound(type: string) {
  withReadyAudio(ctx => {
    ...
    if (normalized === "good") {
      playSequence(ctx, [440, 554, 659, 880], "triangle", now, 0.36, 0.18, 1500, 1.1)
      playSequence(ctx, [988, 1108], "sine", now + 0.16, 0.36, 0.09, 1900, 0.88)
      playSequence(ctx, [220], "square", now, 0.75, 0.04, 450, 1)
    } else if (normalized === "bad") {
      playSequence(ctx, [330, 311, 294, 262], "triangle", now, 0.38, 0.15, 850, 1.6)
      playSequence(ctx, [196], "sine", now + 0.16, 0.38, 0.08, 500, 1)
      playSequence(ctx, [110], "sine", now + 0.1, 0.5, 0.04, 300, 1)
    } else {
      playSequence(ctx, [330, 392, 440, 554], "triangle", now, 0.32, 0.16, 1100, 1.2)
      playSequence(ctx, [660], "sine", now + 0.14, 0.32, 0.085, 1400, 1)
      playSequence(ctx, [260], "square", now + 0.05, 0.44, 0.045, 420, 0.95)
    }
  })
}
